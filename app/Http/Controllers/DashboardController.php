...
//        }
        //License Check
//        if ($this->panelInit->license->data->status == 0) {
           // die($this->panelInit->license->data->message);
//        }
    }
    ...
    public function dashboardData() {
        ...
        foreach ($newsboard as $event) {
            ...
            $eventsArray['sort_key'] = str_replace('/', '', $eventsArray['start']) + $counter++;
            $toReturn['newsEvents'][str_replace('/', '', $eventsArray['start']) + $counter++] = $eventsArray;
        }
        ...
        $toReturn['newsEvents'] = collect($toReturn['newsEvents'])->sortBy('sort_key')->reverse()->toArray();
    ...
    public function profileImage($id) {
        ...
        if (file_exists('uploads/profile/profile_' . $id . '.jpg')) {
            $img = 'uploads/profile/profile_' . $id . '.jpg';
        }
        if (!file_exists($img)) {
            $user = DB::table('users')->where('id', $id)->first();
            $img = 'uploads/profile/profile_' . $user->role . '_' . $user->gender . '.png';
        }
        if (!file_exists($img)) {
            $img = 'uploads/profile/user.png';
        }
    ...
    public function teacherList() {
        if ($this->data['users']->role == "teacher") {
            return \User::where('id', $this->data['users']->id)->get()->toArray();
        } else {
            return \User::where('role', 'teacher')->get()->toArray();
        }
    }
    ...
    public function teacherClassesList() {
        $teacher = isset($_GET['teacher']) ? $_GET['teacher'] : 0;
        $toReturn = array();
        $classesList = array();
        $classesList = \classes::where('classTeacher', 'like', '%"' . trim($teacher) . '"%')->get()->toArray();
        $toReturn['classes'] = $classesList;
        return $toReturn;
    }
    ...
    public function classSubjectsList() {
        ...
        $toReturn = array();
        $subjectsList = array();
        $classModel = \classes::where('id', $class)->first();
        preg_match_all('!\d+!', $classModel->classSubjects, $subjects);
        $subjectsList = \subject::where('teacherId', 'like', '%"' . trim($teacher) . '"%')->whereIn('id', $subjects[0])->get()->toArray();
        $toReturn['subjects'] = $subjectsList;
        return $toReturn;
    }
...
//                $eventsArray['url'] = "portal#assignments";
                $eventsArray['url'] = "portal#agenda/" . $event->AssignDeadLine;
...
//                $eventsArray['url'] = "portal#homework";
                $eventsArray['url'] = "portal#agenda/" . $event->homeworkSubmissionDate ;
                $eventsArray['allDay'] = true;
                $toReturn[] = $eventsArray;
            }
        }
...
//            $eventsArray['url'] = "portal#events/" . $event->id;
                $eventsArray['url'] = "portal#agenda/" . $event->eventDate;
            $eventsArray['textColor'] = '#fff';
            $eventsArray['allDay'] = true;
            $toReturn[] = $eventsArray;
        }
...
//            $eventsArray['url'] = "portal#examsList";
                $eventsArray['url'] = "portal#agenda/" . $event->examDate ;
            $eventsArray['textColor'] = '#fff';
            $eventsArray['allDay'] = true;
            $toReturn[] = $eventsArray;
        }
...
//                $eventsArray['url'] = "portal#agenda/" . $eventsArray['date'] ;
                $eventsArray['url'] = "portal#agenda/" . $eventsArray['date'] ;
...
//                $eventsArray['url'] = "portal#calender";
                $eventsArray['url'] = "portal#agenda/" . $date ;
                $eventsArray['textColor'] = '#fff';
                $eventsArray['allDay'] = true;
                $toReturn[] = $eventsArray;
            }
