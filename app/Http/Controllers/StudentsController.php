...
class StudentsController extends Controller {
    ...
    public function import($type) {
        ...
        if (\Input::hasFile('excelcsv')) {
            ...
            if ($_FILES['excelcsv']['tmp_name']) {
                $readExcel = \Excel::load($_FILES['excelcsv']['tmp_name'], function($reader) {
    ...
    function marksheets($id) {
        
        //Class
        $class = \classes::where('id', $id)->first();

        //Subjects List
        $subjectsList = \subject::whereIn('id', json_decode($class['classSubjects']))->get()->toArray();
        $subjectsArray = [];
        foreach ($subjectsList as $subject) {
            $subjectsArray[] = $subject['subjectTitle'];
        }
        $marks = $this->calcMarks($id);
        return [
            'marks' =>$marks,
    ...
    function marksheetsPDF($id) {
        $marks = $this->calcMarks($id);
        //Students List
        $studentsList = \User::where('studentAcademicYear', $this->panelInit->selectAcYear)->where('role', 'student')->where('studentClass', 'LIKE', '%' . $id . '%')->get()->toArray();
        $studentsArray = [];
		...
		<table cellspacing=\"5\" cellpadding=\"4\" border=\"0\">
			...
			<table cellspacing='0' style='padding: 1px; width: 100%; font-size: 11pt; '>
	            <tr>
	                <th style='width: 100%; text-align: center; '> <b>" . $marks[$student['id']]['name'] . "</b> </th>
	            </tr>
			</table>
			<br/><br/>
            ...
            <table cellspacing=\"0\" cellpadding=\"4\" border=\"1\">
                <tbody><tr>
                    ...
                    <th style='width:15%;border: solid 1px #000000; padding:2px;'>" . $this->panelInit->language['gradeItems'] . "</th>
                    <th style='width:10%;border: solid 1px #000000; padding:2px;'>" . $this->panelInit->language['examMarks'] . "</th>
                    <th style='width:12%;border: solid 1px #000000; padding:2px;'>" . $this->panelInit->language['totalMarks'] . "</th>
                    <th style='width:12%;border: solid 1px #000000; padding:2px;'>" . $this->panelInit->language['status'] . "</th>
                </tr>";

            foreach ($marks[$student['id']]['data'] as $value) {
                $content .= "<tr>
	                    <td style='border: solid 1px #000000;padding:2px;'>" . $value['subjectTitle'] . "</td>
                            <td style='border: solid 1px #000000;padding:2px;'>" . $value['subjectFinalGrade'] . "</td>
	                    <td style='border: solid 1px #000000;padding:2px;'>" . $value['subjectpassGrade'] . "</td>
	                    <td style='border: solid 1px #000000;padding:2px;'>" . $value['itemMark'] . "</td>
	                    <td style='border: solid 1px #000000;padding:2px;'>" . $value['examMark'] . "</td>
	                    <td style='border: solid 1px #000000;padding:2px;'>" . $value['finalMark'] . "</td>
	                    <td style='border: solid 1px #000000;padding:2px;'>" . $value['status'] . "</td>
	                </tr>";
            }
            $content .= "</tbody></table>
			...
			<table cellspacing='0' style='padding: 1px; width: 100%; font-size: 10pt; '>
	            <tr>
	                <th style='width: 100%; text-align: center; '> " . $this->panelInit->language['examMarks'] . " : " . " - " . " </th>
	            </tr>
			</table>
			<br/><br/>";

            $pdfbuilder->table($content, array('border' => '0', 'align' => ''));
            if ($countPages != count($studentsList)) {
                $pdfbuilder->addPage();
            }
        }
        $pdfbuilder->output('Marksheets.pdf');
    ...
    function marksheet($id) {
        ...
        $item_marks = array();
        $itemIds = array();
        $itemsList = \App\Models\grade_items::where('itemAcYear', $this->panelInit->selectAcYear)->where('itemClasses', 'LIKE', '%' . $studentClass . '%')->get();
        foreach ($itemsList as $item) {
            $item_marks[$item->id] = array("title" => $item->itemTitle, "itemId" => $item->id, "studentId" => $id);
            $itemIds[] = $item->id;
        }



        if (count($examIds) == 0 && count($itemIds) == 0) {
            return $this->panelInit->apiOutput(false, $this->panelInit->language['Marksheet'], $this->panelInit->language['studentHaveNoMarks']);
            exit;
        }
        ...
        //Item Marks
        $itemMarks = \App\Models\item_marks::where('studentId', $id)->whereIn('itemId', $itemIds)->get();

        if (count($examMarks) == 0 && count($itemMarks) == 0) {
            return $this->panelInit->apiOutput(false, $this->panelInit->language['Marksheet'], $this->panelInit->language['studentHaveNoMarks']);
            exit;
        }
    ...
    public function calcMarks($id) {
        ...
        $studentsIds = array_column($studentsList, 'id', 'id');
        //Subjects List
        $subjectsList = \subject::whereIn('id', json_decode($class['classSubjects']))->get()->toArray();
        $subjectsArray = [];
        $init = [];
        foreach ($subjectsList as $subject) {
            ...
            $init[$subject['id']] = [
                ...
                'subjectFinalGrade' => $subject['finalGrade'],
                'subjectpassGrade' => $subject['passGrade'],
                'examWeight' => 100,
                'examMark' => sprintf('%.2f', 0),
                'itemWeight' => 100,
                'itemMark' => sprintf('%.2f', 0),
                'finalMark' => sprintf('%.2f', 0),
                'status' => 'N/A'
            ];
        }
        ...
        $marks = [];
        //Item List
        $itemsList = \App\Models\grade_items::where('itemAcYear', $this->panelInit->selectAcYear)->where('itemClasses', 'LIKE', '%' . $id . '%')->get();
        $studentItemsMarks = [];
        foreach ($itemsList as $item) {
            $itemMarks = \App\Models\item_marks::where('itemId', $item->id)->where('classId', $id)->whereIn('studentId', $studentsIds)->get()->toArray();
            foreach ($itemMarks as $itemMark) {
//                $studentItemsMarks[$itemMark['studentId']][$itemMark['subjectId']]['mark'] += $itemMark['totalMarks'] / $item['maxMark'] * $item['itemWeight'];
                if (!isset($studentItemsMarks[$itemMark['studentId']])) {
                    $studentItemsMarks[$itemMark['studentId']] = [];
                }
                if (!isset($studentItemsMarks[$itemMark['studentId']][$itemMark['subjectId']])) {
                    $studentItemsMarks[$itemMark['studentId']][$itemMark['subjectId']] = [];
                }
                if (!isset($studentItemsMarks[$itemMark['studentId']][$itemMark['subjectId']])) {
                    $studentItemsMarks[$itemMark['studentId']][$itemMark['subjectId']] = [];
                }
                if (!isset($studentItemsMarks[$itemMark['studentId']][$itemMark['subjectId']]['mark'])) {
                    $studentItemsMarks[$itemMark['studentId']][$itemMark['subjectId']]['mark'] = 0;
                }
                if (!isset($studentItemsMarks[$itemMark['studentId']][$itemMark['subjectId']]['weight'])) {
                    $studentItemsMarks[$itemMark['studentId']][$itemMark['subjectId']]['weight'] = 0;
                }
                $studentItemsMarks[$itemMark['studentId']][$itemMark['subjectId']]['mark'] += $itemMark['totalMarks'] * $item['itemWeight'] / $itemMark['maxMark'];
                $studentItemsMarks[$itemMark['studentId']][$itemMark['subjectId']]['weight'] += $item['itemWeight'];
            }
        }
        //Exam List
        $examsList = \exams_list::where('examAcYear', $this->panelInit->selectAcYear)->where('examClasses', 'LIKE', '%' . $id . '%')->get();
        $examsArray = [];
        $examsIds = [];
        foreach ($examsList as $exam) {
            $examsArray[$exam['id']] = $exam;
            $examsIds[] = $exam['id'];
        }

        $examMarks = \exam_marks::whereIn('examId', $examsIds)->where('classId', $id)->whereIn('studentId', $studentsIds)->get()->toArray();
        $passed = 0;
        $failed = 0;
        foreach ($examMarks as $examMark) {
            $examWeight = $examsArray[$examMark['examId']]['examWeight'] / 100;
            $itemWeight = 1 - $examWeight;
            $exam_mark = $examMark['totalMarks'] * $examWeight;
            if(isset($studentItemsMarks[$examMark['studentId']][$examMark['subjectId']])){
            $item_mark = $studentItemsMarks[$examMark['studentId']][$examMark['subjectId']]['mark'] / 
                            $studentItemsMarks[$examMark['studentId']][$examMark['subjectId']]['weight'] *
                            $subjectsArray[$examMark['subjectId']]['finalGrade'] * $itemWeight;
            }else{
                $item_mark = 0;
            }
            $final_mark = $item_mark + $exam_mark;
            if ($final_mark > intval($subjectsArray[$examMark['subjectId']]['passGrade'])) {
                $status = 'Passed';
                $passed++;
            } else {
                $status = 'Failed';
            ...
            $marks[$examMark['studentId']]['name'] = $studentsArray[$examMark['studentId']]['fullName'];
            $marks[$examMark['studentId']]['studentId'] = $examMark['studentId'];
            $marks[$examMark['studentId']]['examId'] = $examMark['examId'];
            $marks[$examMark['studentId']]['data'] = $init;
            $marks[$examMark['studentId']]['data'][$examMark['subjectId']] = [
                'subjectTitle' => $subjectsArray[$examMark['subjectId']]['subjectTitle'],
                'subjectFinalGrade' => $subjectsArray[$examMark['subjectId']]['finalGrade'],
                'subjectpassGrade' => $subjectsArray[$examMark['subjectId']]['passGrade'],
                'examWeight' => $examWeight,
                'examMark' => sprintf('%.2f', $exam_mark),
                'itemWeight' => $itemWeight,
                'itemMark' => sprintf('%.2f', $item_mark),
                'finalMark' => sprintf('%.2f', $final_mark),
                'status' => $status
            ];
        }

        $statistics = [
            'studentsNumber' => count($marks),
            'passed' => $passed,
            'failed' => $failed,
        ];
        return $marks;
