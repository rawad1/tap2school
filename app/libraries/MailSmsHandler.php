...
class MailSmsHandler {
	...
	public function custom($to,$message){
                ...
                if (!isset($this->settings['customHTTPURL']) || !isset($this->settings['customHTTPParams']) || !isset($this->settings['customHTTPType']) || !isset($this->settings['customHTTPToFormat']))
            return;
        if ($this->settings['customHTTPURL'] == "" || $this->settings['customHTTPParams'] == "" || $this->settings['customHTTPType'] == "" || $this->settings['customHTTPToFormat'] == "")
            return;
        ...
        $arr = explode('&', $this->settings['customHTTPParams']);
        $params = [];
        foreach ($arr as $param) {
            $x = explode('=', $param);
            $params[$x[0]] = $x [1];
        }
        $params['destination'] = trim($to);
        $params['text'] = htmlspecialchars_decode(trim($message), ENT_QUOTES);
        curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($params));
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        if (isset($this->settings['customHTTPHeaders']) AND $this->settings['customHTTPHeaders'] != "") {
            $array = explode("|", $this->settings['customHTTPHeaders']);
